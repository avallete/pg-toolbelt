...
export const handleDeclarativeApply = (args: DeclarativeApplyHandlerArgs) =>
  Effect.gen(function* () {
    ...
    const onRoundComplete = args.verbose
      ? (round: RoundResult) => {
          ...
          void output.info(parts.join("  ")).pipe(Effect.runPromise);
        }
      : undefined;
      ...
      for (const diagnostic of warnings) {
        ...
        if (
          ...
          yield* Effect.tryPromise(() =>
            resolveSqlFilePath(args.path, statementId.filePath).then(
              (fullPath) =>
        ...
        if (apply.validationErrors && apply.validationErrors.length > 0) {
          ...
          for (const error of apply.validationErrors) {
            const formatted = yield* Effect.promise(() =>
              formatStatementError(error, args.path),
            );
